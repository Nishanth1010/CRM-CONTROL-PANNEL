...
const FollowupList: React.FC = () => {
  ...
  const fetchLeads = () => {
    ...
    // 🔥 Update the status filter to only fetch 'new' and 'in_progress'
    if (statusFilter) {
      // If user applied a custom status filter (via UI), use that
      params.status = statusFilter;
    } else {
      // Otherwise, default to only fetching 'new' and 'in_progress'
  ...
  const filteredLeads = sortedLeads.filter(
    (lead) =>
      !['customer', 'rejected'].includes(lead.status.trim().toLowerCase()) &&
